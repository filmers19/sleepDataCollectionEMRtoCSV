...
def preprocess_for_document_mask(gray: np.ndarray) -> tuple[np.ndarray, np.ndarray, int]:
    # Estimate large-scale illumination and normalize it out so the page remains bright
    # even when the background is dark or the lighting is uneven.
    ...
    # "Black" is defined relative to the page-normalized image. Anything below the Otsu split
    # is treated as non-document / dark structure for the page mask step.
