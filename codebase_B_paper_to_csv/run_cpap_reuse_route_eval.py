...
OUTPUT_ROOT = ROOT / "out_cpap_reuse_route_eval_20260319"
...
def _cpap_keys(output_columns: Sequence[str]) -> List[str]:
    keys: List[str] = []
    for key in output_columns:
        if key.startswith("Pressure_"):
            keys.append(key)
        elif key.startswith("Pr") and len(key) >= 4 and key[2:4].isdigit():
            keys.append(key)
    return keys
...
def _discover_cpap_pages(pipeline_mod: Any) -> Dict[str, List[Dict[str, Any]]]:
    out: Dict[str, List[Dict[str, Any]]] = {}
    for patient_dir in sorted(SOURCE_ROOT.glob("Patient_*")):
        pages: List[Dict[str, Any]] = []
        map_dir = patient_dir / "map_pages"
        if not map_dir.exists():
            continue
        for meta_path in sorted(map_dir.glob("*.meta.json")):
            try:
            ...
            if route_name not in {
                str(pipeline_mod.MAP_ROUTE_CPAP_PSG_REPORT_GENERAL),
                str(pipeline_mod.MAP_ROUTE_CPAP_PSG_REPORT_EXTENSIVE),
            }:
                continue
            bundle_name = str(meta.get("bundle") or meta_path.stem)
            ...
            ocr_text = (patient_dir / "ocr_pages" / f"{Path(source_image).stem}.txt").read_text(encoding="utf-8")
            pages.append(
                {
        ...
        if pages:
            out[patient_dir.name] = pages
    return out


async def amain() -> None:
    pipeline_mod = _load_module(PIPELINE_PATH, "paper_to_cdm_sa_cpap_reuse")
    unified_mod = _load_module(UNIFIED_PATH, "unified_runner_cpap_reuse")
    ...
    cpap_keys = _cpap_keys(output_columns)
    reference_rows = _read_csv_rows(EXAMPLE_CSV)
    cpap_pages_by_patient = _discover_cpap_pages(pipeline_mod)
    target_patients = sorted(cpap_pages_by_patient.keys())
    ...
    try:
        for patient_name in target_patients:
            ...
            target_pages = cpap_pages_by_patient[patient_name]
            for item in target_pages:
                ...
                raw_obj, valid_obj, valid_contexts, valid_cdm_contexts, rejected_fields = await unified_mod.map_ocr_text_multi_agent(
                    ...
                    map_agent_count_by_route={
                        str(pipeline_mod.MAP_ROUTE_CPAP_PSG_REPORT_GENERAL): 2,
                        str(pipeline_mod.MAP_ROUTE_CPAP_PSG_REPORT_EXTENSIVE): 2,
                    },
                    route_name=route_name,
            ...
            new_eval = _evaluate_subset(new_row, ref_row, cpap_keys)
            baseline_eval = _evaluate_subset(baseline_row, ref_row, cpap_keys)

            (patient_out / "cpap_eval_details.json").write_text(
                json.dumps(
                    {
            ...
            patient_summary = {
                "patient_name": patient_name,
                "cpap_key_count": new_eval["key_count"],
