...
OUTPUT_ROOT = ROOT / "out_questionnaire_reuse_routeclean_20260318"
ROUTE_LABEL_CSV = ROOT / "manual_route_labels_latest10patients_20260318.csv"
...
def _find_route_clean_patients() -> List[str]:
    rows = _read_csv_rows(ROUTE_LABEL_CSV)
    by_patient: Dict[str, List[int]] = {}
    for row in rows:
        by_patient.setdefault(row["patient"], []).append(int(row["match"]))
    return sorted(patient for patient, vals in by_patient.items() if vals and all(v == 1 for v in vals))
...
async def amain() -> None:
    pipeline_mod = _load_module(PIPELINE_PATH, "paper_to_cdm_sa_questionnaire_routeclean")
    unified_mod = _load_module(UNIFIED_PATH, "unified_runner_questionnaire_routeclean")
    ...
    target_patients = _find_route_clean_patients()
    ...
    try:
        ...
        aggregate = {
            ...
            "cdm_csv": str(CDM_CSV),
            "route_label_csv": str(ROUTE_LABEL_CSV),
