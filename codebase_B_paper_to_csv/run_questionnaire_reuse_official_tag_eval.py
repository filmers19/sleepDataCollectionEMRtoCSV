...
OUTPUT_ROOT = ROOT / "out_questionnaire_reuse_officialtag_psqiwrong_20260318"
...
def _find_target_patients() -> List[str]:
    run_rows = _read_csv_rows(SOURCE_ROOT / "all_patients.csv")
    ref_rows = _read_csv_rows(EXAMPLE_CSV)
    if not run_rows or not ref_rows:
        return []
    keys = [k for k in run_rows[0].keys() if any(k.startswith(prefix) for prefix in PSQI_0104_PREFIXES)]
    out: List[str] = []
...
async def amain() -> None:
    pipeline_mod = _load_module(PIPELINE_PATH, "paper_to_cdm_sa_questionnaire_reuse")
    unified_mod = _load_module(UNIFIED_PATH, "unified_runner_questionnaire_reuse")
    ...
    try:
        for patient_name in target_patients:
            ...
            old_row = _read_csv_rows(source_dir / f"{patient_name}.csv")[0]

            new_eval = _evaluate_subset(new_row, ref_row, questionnaire_keys)
            old_eval = _evaluate_subset(old_row, ref_row, questionnaire_keys)

            mismatch_details = {
                "new_mismatches": new_eval["mismatches"],
                "old_mismatches": old_eval["mismatches"],
            }
            ...
            patient_summary = {
                ...
                "old_semantic_accuracy": old_eval["semantic_accuracy"],
                "old_precision": old_eval["precision"],
                "old_recall": old_eval["recall"],
                "old_f1": old_eval["f1"],
                "old_mismatches": len(old_eval["mismatches"]),
                "semantic_accuracy_gain": new_eval["semantic_accuracy"] - old_eval["semantic_accuracy"],
                "precision_gain": new_eval["precision"] - old_eval["precision"],
                "recall_gain": new_eval["recall"] - old_eval["recall"],
                "f1_gain": new_eval["f1"] - old_eval["f1"],
                "mismatch_delta": len(old_eval["mismatches"]) - len(new_eval["mismatches"]),
            }
        ...
        aggregate = {
            ...
            "avg_new_semantic_accuracy": float(pd.DataFrame(patient_summaries)["new_semantic_accuracy"].mean()) if patient_summaries else None,
            "avg_old_semantic_accuracy": float(pd.DataFrame(patient_summaries)["old_semantic_accuracy"].mean()) if patient_summaries else None,
            "avg_semantic_accuracy_gain": float(pd.DataFrame(patient_summaries)["semantic_accuracy_gain"].mean()) if patient_summaries else None,
            "total_new_mismatches": int(pd.DataFrame(patient_summaries)["new_mismatches"].sum()) if patient_summaries else 0,
            "total_old_mismatches": int(pd.DataFrame(patient_summaries)["old_mismatches"].sum()) if patient_summaries else 0,
            "total_mismatch_delta": int(pd.DataFrame(patient_summaries)["mismatch_delta"].sum()) if patient_summaries else 0,
